  ...
  return (
    <div className='container'>
       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {Popular.map((list) => (
            <div key={list.id}  className="w-[300px] h-[450px] group rounded-2xl hover:bg-blueDark duration-200 ease-in bg-[#edebeb] group relative max-w-xs overflow-hidden bg-cover bg-no-repeat">
              ...
              <p className="group-hover:text-white ">{list.p}</p>
                ...
                <div className="flex justify-center px-2 pt-7">
                  <button className='rounded-full border-black border-2 w-full bg-black text-white group-hover:bg-white group-hover:text-black py-1'>{list.span}</button>
                </div>
            </div>
            ))
        }
